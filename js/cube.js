...
$(document).ready(function () {
  ...
  // ✅ Load cube type (first from sessionStorage, then JSONBin)
  const localCubeType = sessionStorage.getItem("cubeType");
  if (localCubeType) {
    $("#cubeTypeSelector").val(localCubeType).trigger("change");
    changeCubeImages(localCubeType);
  } else {
    // load from JSONBin if no local session value
    $.ajax({
      url: `${BIN_URL}/latest`,
      headers: {
        "X-Access-Key": ACCESS_KEY,
      ...
      success: function (res) {
        ...
        $("#cubeTypeSelector").val(cubeType).trigger("change");
        changeCubeImages(cubeType);
        sessionStorage.setItem("cubeType", cubeType); // ✅ store locally
      },
      error: function () {
        changeCubeImages("kids");
      }
  ...
  $("#cubeTypeSelector").on("change", function () {
    ...
    changeCubeImages(selectedType);
    saveCubeType(selectedType);
    sessionStorage.setItem("cubeType", selectedType); // ✅ update local session
  });
